...
namespace linal
{
  ...
  public:
      ...
      virtual const std::vector<int32_t> &shape() const noexcept  = 0;
  ...
  template<typename T,int32_t _dim_src,int32_t _dim_dst>
  thensor<T,_dim_dst> reshape(const thensor<T,_dim_src> &src,const std::vector<int32_t> &new_shape);
  template<typename T, int32_t _dim>
  thensor<T, _dim>& zero_set(thensor<T, _dim>&);
  template<typename T, int32_t _dim, typename... Args>
  ...
  public:
      typedef T d_type;
      static const int32_t d_dim = _dim;
      ...
      template<typename S, int32_t d>
      friend thensor<S, d> &operator<<(thensor<S, d> &left, const thensor<S, d> &right);
      template<typename U, typename S, int32_t d>
      ...
      thensor<T, _dim - 1> operator[](int32_t idx) const;
      
      inline int32_t size() const noexcept { return _data.size(); };
      inline const T *data() const noexcept { return _data.data(); }
      inline T *data() noexcept { return _data.data(); }
      inline const std::vector<int32_t> &shape() const noexcept { return _data.shape(); };
      
      //this method initialized data from external source
      //it means that data will not be allocated or free-ed inside
	  thensor<T, _dim>& wrap(T *data, const std::vector<int32_t> &shapes) { _data.construct(data, shapes); return *this; };

	  thensor<T, _dim>& reshape(const std::vector<int32_t> &new_shape) { _data.reshape(new_shape); return *this; };
      ...
      explicit thensor(const std::vector<int32_t> &shapes) : _data(shapes) {};
      ~thensor() = default;
  };
  ...
  public:
      ...
      //this method initialized data from external source
      //it means that data will not be allocated or free-ed inside
	  thensor<T, 1>& wrap(T *data, const std::vector<int32_t> &shapes) { _data.construct(data, shapes);  return *this; };
      ...
      thensor() = default;
      explicit thensor(const std::vector<int32_t> &shapes) : _data(shapes) {};
      explicit thensor(int32_t len) : _data({len}) {};
  ...
  template<typename U, typename S, int32_t _dim>
  ...
  thensor<T, 1> operator*(const thensor<T, 2> &left, const thensor<T, 1> &right)
  {
      ...
      int32_t outShape = left.shape()[0];
      int32_t inShape = right.shape()[0];
      thensor<T,1> resVec(outShape);
      T*  res = resVec.data();
      const T* vec = right.data();
      const T* src = left.data();
      for (int32_t i =0; i < outShape; i++)
      {
          T t= T();
          
          for (int32_t j =0 ; j < inShape; j++,src++)
          {
              t+=vec[j]*(*src);
          }
          res[i] = t;
  ...
  std::ostream &operator<<(std::ostream &os, const thensor<T, _dim> &th)
  {
      std::vector<int32_t> shape = th.shape();
      for(int32_t i = 0 ; i <th.size(); i++)
      {
          int32_t k = 1;
          if (i != 0)
          {
              for (int32_t j = _dim - 1; j >= 0, i % (k * shape[j]) == 0; --j)
              {
                  os << '\n';
                  k*=shape[j];
              }
  ...
  template<typename T, int32_t _dim>
  inline thensor<T, _dim - 1> thensor<T, _dim>::operator[](int32_t idx) const
  {
  ...
  template<typename T>
  inline T& thensor<T, 1>::operator[](int32_t idx) const
  {
      return _data[idx];
  }
  ...
  thensor<T, 2> transpose(const thensor<T, 2> &left)
  {
      ...
      T* dst = res.data();
      for (int32_t i = 0 ; i < shp[1]; i++,col++)
      {
          const T* src =col;
          for (int32_t j = 0; j < shp[0]; j++,src+=shp[1],dst++)
          {
              *dst = *src;
          }
      }
